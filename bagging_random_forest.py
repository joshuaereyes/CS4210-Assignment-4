...
# SPECIFICATION: build a base classifier using single decision tree
...
#--> add your Python code here
for i, c in enumerate(classVotes):
   max = 0
   for j in range(len(c)):
      if c[j] > c[max]:
         max = j
   if(int(max) == int(dbTest[i][-1])):
      correct+=1
...
#--> add your Python code here
class_predicited_rf = clf.predict([row[:-1] for row in dbTest])

#compare the Random Forest prediction for each test sample with the ground truth label to calculate its accuracy
#--> add your Python code here
accuracy_rf = 0
for i in range(len(dbTest)):
   if class_predicited_rf[i] == dbTest[i][-1]:
      accuracy_rf += 1
accuracy_rf = accuracy_rf/ len(dbTest)
